...
export const addStaff = async (businessId, nombre, numero, password, avatar = null, apellido = "", staffId = null, options = {}) => {
    try {
        ...
        const rawPermissions = options &&
            typeof options.permissions === "object" &&
            options.permissions !== null
            ? options.permissions
            : {};
        const permissions = {
            manualAppointments: Boolean(rawPermissions.manualAppointments ??
            ...
            viewClientPhone: Boolean(rawPermissions.viewClientPhone ??
        ...
        const staff = staffQuery.docs.map((doc) => {
            ...
            return {
                ...
                permissions: {
                    manualAppointments: Boolean(rawPermissions.manualAppointments ??
                        DEFAULT_STAFF_PERMISSIONS.manualAppointments),
                    manualBlocks: Boolean(rawPermissions.manualBlocks ??
...
export const getBusinessUserByNumero = async (numero) => {
    try {
        ...
        const setupFlag = typeof userData.isInitialSetupComplete === "string"
    ...
    try {
        ...
        return await db.runTransaction(async (transaction) => {
            const userQuery = await db
                ...
                .get();
            if (userQuery.empty)
                throw new Error("Usuario de negocio no encontrado");
            const userDoc = userQuery.docs[0];
            const userRef = userDoc.ref;
            const userData = userDoc.data();
            const now = Date.now(); // � usamos timestamp numérico local
        ...
        if (permissions !== undefined) {
            if (permissions && typeof permissions === "object") {
                ...
                updateData.permissions = {
                    manualAppointments: Boolean(permissions.manualAppointments ??
                        current.manualAppointments ??
                        DEFAULT_STAFF_PERMISSIONS.manualAppointments),
                    manualBlocks: Boolean(permissions.manualBlocks ??
                        current.manualBlocks ??
                        DEFAULT_STAFF_PERMISSIONS.manualBlocks),
                    viewClientPhone: Boolean(permissions.viewClientPhone ??
...
export const createAppointment = async ({ businessId, staffId, userId, serviceId, date, horario, calificacion, }) => {
    try {
        ...
        const appointment = {
            ...
            serviceId: serviceId !== undefined && serviceId !== null
                ? Number(serviceId)
                : null,
...
export const updateAppointmentCalificacion = async (appointmentsId, calificacion, descripcion) => {
    try {
        ...
        const now = new Date();
        const calificacionUpdatedAt = `${String(now.getDate()).padStart(2, "0")}/${String(now.getMonth() + 1).padStart(2, "0")}/${now.getFullYear()}`;
        const patch = {
            calificacion: calificacion === null || calificacion === undefined
                ? null
                : Number(calificacion),
            descripcion: descripcion === null || descripcion === undefined
                ? null
                : String(descripcion),
    ...
    try {
        ...
        const results = querySnap.docs.map((d) => {
            ...
            return {
                ...
                calificacion: data.calificacion === null || data.calificacion === undefined
                    ? null
                    : Number(data.calificacion),
                staffNombre: staffInfo.nombre,
                staffApellido: staffInfo.apellido,
                service: svcInfo,
                userId: typeof data.userId === "number"
                    ? data.userId
                    : data.userId
                        ? Number(data.userId)
                        : null,
...
export const getAppointmentsByClientId = async (clientId) => {
    try {
        ...
        const querySnap = await db
            ...
            .get();
        const businessIds = Array.from(new Set(querySnap.docs
        ...
        if (businessIds.length > 0) {
            ...
            const snaps = await Promise.all(chunks.map((chunk) => db.collection("user-business").where("id", "in", chunk).get()));
            snaps.forEach((snap) => {
...
export const getListClientsByBusiness = async (businessId) => {
    try {
        ...
        const now = new Date();
        const todayEndEpoch = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999).getTime();
        for (const appt of appointments) {
            const uid = appt.userId;
            if (uid == null)
                continue;
            const apptEpoch = toEpoch(appt.staffdates);
            if (apptEpoch > todayEndEpoch)
                continue;
        ...
        for (const rec of byUser.values()) {
            ...
            if (rec.count > 3)
                ...
                noTeVisitan.push(item);
            else
                noHanVuelto.push(item);
        }
...
export const createStaffSchedule = async (businessId, staffId, payload) => {
    try {
        ...
        if (!bizIdNum || !stid)
            throw new Error("businessId y staffId requeridos");
...
export const getBusinessById = async (businessId) => {
    try {
        ...
        return {
            ...
            direccion: data?.direccion && typeof data.direccion === "object"
                ? data.direccion
                : null,
...
export const updateClientById = async (clientId, updateData = {}) => {
    try {
        ...
        if (updateData.nombre !== undefined)
            patch.nombre = String(updateData.nombre ?? "");
        if (updateData.apellido !== undefined)
            patch.apellido = String(updateData.apellido ?? "");
        if (updateData.numero !== undefined)
            patch.numero = String(updateData.numero ?? "");
        if (updateData.avatar !== undefined)
            patch.avatar = updateData.avatar ?? null;
        patch.updatedAt = admin.firestore.FieldValue.serverTimestamp();
        await ref.update(patch);
        const updated = await ref.get();
...
export const createService = async (businessId, name, type, duration, price, category, description, promotionTerms, promotionValidUntil, promotionValidIndefinite) => {
    try {
        ...
        if (catStr === "promotion") {
            ...
            const untilStr = promotionValidUntil !== undefined && promotionValidUntil !== null
...
export const updateService = async (businessId, serviceId, updateData) => {
    try {
        ...
        if (updateData.name !== undefined)
            patch.name = String(updateData.name);
        if (updateData.type !== undefined)
            patch.type = String(updateData.type);
        ...
        const hasPromoFields = updateData.promotionTerms !== undefined ||
        ...
        staffSnap.docs.forEach((sd) => {
            ...
            if (ss.includes(sidNum)) {
                const cfg = d.staffServiceConfigs && d.staffServiceConfigs[String(sidNum)]
...
export const getGeneralServicesByBusiness = async ({ page = 1, limit = 10 }) => {
    try {
        ...
        if (paginatedBusinessIds.length > 0) {
            ...
            const businessSnaps = await Promise.all(chunks.map((chunk) => db.collection("user-business").where("id", "in", chunk).get()));
        ...
        if (paginatedBusinessIds.length > 0) {
            ...
            const staffSnaps = await Promise.all(chunks.map((chunk) => db.collection("staff").where("businessId", "in", chunk).get()));
...
export const updateBusinessPolicies = async (businessId, { cancellationAdvanceMinutes, minAdvanceBookingMinutes, reminderMinutes }) => {
    try {
        ...
        if (c !== undefined)
            polPatch["policies.cancellationAdvanceMinutes"] = c;
        if (a !== undefined)
            polPatch["policies.minAdvanceBookingMinutes"] = a;
        if (r !== undefined)
            polPatch["policies.reminderMinutes"] = r;
...
export const getExpenseCategoryById = (categoryId) => {
    const categories = getExpenseCategories().reduce((acc, c) => ({ ...acc, [c.id]: c }), {});
    const category = categories[categoryId];
    if (!category) {
        throw new Error("Categoría de gasto inválida. Use un id entre 1 y 7 (obtener listado desde GET /expense-categories)");
    }
    return category;
};
...
export const createExpense = async ({ businessId, name, category, categoryId, paidAt, amount, }) => {
    try {
        ...
        const catIdNum = categoryId !== undefined && categoryId !== null
            ? Number(categoryId)
            : undefined;
        if (catIdNum === undefined || !Number.isFinite(catIdNum)) {
            throw new Error("categoryId requerido (obtener listado desde GET /expense-categories)");
        }
        let catName;
...
export const getExpensesByBusiness = async (businessId, { year, week } = {}) => {
    try {
        ...
        if (typeof year === "number" && typeof week === "number") {
            return list.filter((e) => Number(e.isoYear) === Number(year) &&
...
export const updateIncome = async (id, { name, category, categoryId, receivedAt, amount }) => {
    try {
        ...
        if (category !== undefined || categoryId !== undefined) {
            let catIdNum = categoryId !== undefined && categoryId !== null
                ? Number(categoryId)
                : undefined;
    ...
    try {
        ...
        if (startDate || endDate) {
            ...
            incomes = incomes.filter((income) => {
                ...
                if (start && incomeDate < start)
                    return false;
                if (end && incomeDate > end)
                    return false;
                return true;
            });
        }
...
export const getBusinessMostUsedTypes = async (businessId) => {
    try {
        ...
        const mostUsedServicioType = getMostUsedNames(incomes.filter((income) => Number(income.categoryId) === 2));
        const mostUsedProductoType = getMostUsedNames(incomes.filter((income) => Number(income.categoryId) === 1));
        return {
