...
import { createAppointment, getAppointmentsByBusiness, getAppointmentsByClientId, getListClientsByBusiness, updateAppointmentCalificacion, deleteAppointment, updateAppointmentReschedule, updateAppointmentState, APPOINTMENT_STATES, } from "../services/firestoreService.js";
    ...
    try {
        ...
        if (req.body?.serviceDuration !== undefined) {
            return res.status(400).json({
                error: "serviceDuration no debe enviarse en el body. La duración se toma del servicio",
            });
        }
        if (!businessId || !staffId || !date || !horario || !bodyUserId) {
            return res.status(400).json({
                error: "Faltan campos requeridos: businessId, staffId, userId, date, horario",
            });
        }
        ...
        const code = /not found|no pertenece|fecha inválida|serviceDuration|Service/i.test(msg)
            ? 400
            : 500;
...
export const updateAppointmentCalificacionController = async (req, res) => {
    try {
        ...
        if (requester.role === "user") {
            ...
            if (String(apptData.staffAppoinments ?? apptData.staffId ?? "") !==
        ...
        const result = await updateAppointmentCalificacion(appointmentsId, calificacion, descripcion);
